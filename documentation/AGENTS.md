...
#### What qualifies as a subagent task

Use a subagent when the work is parallelizable or can be isolated from the main thread:

- Research: investigate docs, prior art, constraints, edge cases
- Spike: quick prototype to validate approach, APIs, or feasibility
- Implementation: build a self contained component or change set
- Review: code review style pass, risk analysis, tests to add, gaps to close
...
- Tests pass (unit, integration, and any relevant end to end checks)
- Lint and formatting clean (no warnings, no new violations)
- Docs updated (README, runbooks, API docs, comments where needed)
- Rollout plan defined (how it ships, flags, monitoring, ownership)
- Rollback plan defined (how to revert quickly, what success and failure look like)
...
When adding primary action buttons, always include explicit dark mode styling to avoid white-on-white issues:
...
For outline/secondary buttons (these work fine with default styling):
...
- Primary actions: `bg-blue-600` / `dark:bg-blue-600`
- Destructive actions: Use `variant='destructive'`
- Success indicators: `text-green-500` / `bg-green-100`
- Warning/caution: `text-amber-600` / `bg-amber-100`
